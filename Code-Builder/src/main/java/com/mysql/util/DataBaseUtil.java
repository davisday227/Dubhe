...
public class DataBaseUtil {
    ...
    public static ClassInfo parseClassInfo(String tableName) throws SQLException {
        ...
        // 获取表注释
        String tableCommentSql = createTableInfoSql(tableName);
        ResultSet resultSet = statement.executeQuery(tableCommentSql);
        // 构建ClassInfo信息
        ...
        while(resultSet.next()) {
            classInfo.setClassComment(resultSet.getString(2));
        }
        ...
        // 设置主键字段
        String primaryKeySql = createPrimaryKeySql(tableName);
        ResultSet primaryKeySet = statement.executeQuery(primaryKeySql);
        while (primaryKeySet.next()) {
            String idName = primaryKeySet.getString(1);
            Optional<FieldInfo> first = fieldList.stream().filter(x -> x.getColumnName().equals(idName)).findFirst();
            if (first.isPresent()) {
                classInfo.setKey(first.get());
                break;
            }
        }
    ...
    private static String createTableInfoSql(String tableName) {
        return MessageFormat.format("select table_name,table_comment " +
                "from information_schema.TABLES " +
                "where table_name = \"{0}\" and table_schema=\"{1}\"", tableName, GlobleConfig.getGlobleConfig().getDataBase());
    }

    private static String createPrimaryKeySql(String tableName) {
        return MessageFormat.format("select column_name from information_schema.key_column_usage " +
                "where table_name = \"{0}\" and table_schema=\"{1}\"", tableName, GlobleConfig.getGlobleConfig().getDataBase());
    }
